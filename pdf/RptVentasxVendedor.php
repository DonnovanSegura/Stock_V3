<?php 
	...
	$query = "SELECT orders.id_order, orders.client_id, orders.fecha_add, orders.total_neto, orders.descuento, orders.total, orders.metodo, 
	          ...
	          WHERE orders.user_add_id = $vendedor  AND orders.tipo_orden = 2 ";
	$resultado = $connect->query($query);


	$query2 = "SELECT id_user, nombre FROM user WHERE id_user = $vendedor ";
	...
	$pdf->Cell(100,6,'',0,0);
	$pdf->Cell(80,6, 'Vendedor:		'.$row2['nombre'],0,1);
	$pdf->Ln(2);
