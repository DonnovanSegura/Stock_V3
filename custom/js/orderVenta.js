...
$(document).ready(function() {
	...
	$("#editOrderFormVentas").unbind('submit').bind('submit', function() {
		...
		var codCte       = $("#codCte").val();
		//var totalNeto = $("#totalAmountValue").val();
		var descuento    = $("#discount").val();
		//var total     = $("#grandTotalValue").val();
		var metodo       = $("#metoPago").val();
		var totalProduct = $("#grandTotalProductos").val();
		var tipoOrden    = $("#TypeOrder").val();
		var montopay     = $("#paid").val();
		var estadopay    = $("#paymentStatus").val();
		...
		if(codCte && descuento &&  metodo && tipoOrden && montopay && estadopay && fchLiqui && totalProduct) {
			...
			if(validateProduct == true && validateQuantity == true) {
					...
					$.ajax({
						...
						success:function(response) {
							...
							// create order button
							$(".edit-success-messages").html('<div class="alert alert-success">'+
		...
		success:function(response) {
			...
			var tr = '<tr id="row'+count+'" class="'+arrayNumber+'">'+
					...
					'<td style="display:none">'+
						'<input type="text" name="statusProd2[]" id="statusProd2'+count+'" autocomplete="off"  class="form-control" style="display:none"  />'+
					'</td>'+
	...
	if (row) {
		...
		if(productId == "" ){
			...
			$.ajax({
				...
				dataType: 'json',
				success:function(response) {
					$("#statusProd2"+row).val(response.estado);
//-----------------------------------------------------------------------------------------------					
...
//------------------------------------------------------------------------------------------------
					if($("#statusProd2"+row).val() == 1 && $("#clientCatHi2").val() == 'PLATA'){
					...
					} else if($("#statusProd2"+row).val() == 1 && $("#clientCatHi2").val() == 'ORO'){
					...
					} else if ($("#statusProd2"+row).val() == 1 && $("#clientCatHi2").val() == 'BRONCE') {
		...
		if(clientId == "" ){
			...
			$.ajax({
				...
				success:function(response) {
					...
					$('#clientCatHi2').val(categorias[response.categoria]);
